...
- **Ingesta automática (además de la manual):** el mismo motor de causación (`procesar_xml`) sirve a todos los canales — subir a mano es solo uno de ellos:
  1. **Carpeta por lotes:** comando `causar_lote <carpeta>` que procesa de una vez todos los XML descargados en una carpeta (del portal DIAN, del correo, de donde sea) y reporta por archivo: causada / duplicada / error con motivo. Es la base del caso P1.8 (100 facturas del mes) y programable con el mismo scheduler de las alertas.
  2. **Buzón de correo por cliente:** conexión IMAP a una cuenta dedicada (credenciales por `.env`, p. ej. `facturas+<tenant>@dominio`), que lee los correos entrantes, extrae los adjuntos XML/ZIP de las facturas y los pasa por el mismo motor; lo no procesable queda registrado con motivo, nunca se pierde un correo. Los proveedores envían la factura al buzón y aparece causada en la bandeja.
  3. La descarga automática del portal DIAN (arriba) alimenta la carpeta del punto 1 — los tres canales convergen en el mismo flujo con las mismas validaciones (CUFE duplicado, XXE, totales).
- **Causación desde foto (factura física):** el usuario toma una foto de la factura de papel (o sube un PDF escaneado) desde el celular — la UI ya es mobile-first. Un modelo de visión (del mismo catálogo NVIDIA/NIM OpenAI-compatible del punto siguiente, o el proveedor que quede tras la validación) extrae los campos (NIT y nombre del emisor, fecha, número, subtotal, IVA, total, concepto) y los entrega al **mismo flujo de causación existente** (matriz de terceros, clasificación PUC, retenciones, asiento). Reglas: (1) lo extraído por visión entra **siempre como "sugerida"**, nunca automática — el OCR se equivoca y el humano confirma campo por campo en un formulario editable; (2) sin CUFE no hay antiduplicado DIAN: se controla por NIT+número+fecha; (3) minimización de datos: solo viaja la imagen al proveedor de IA, y se documenta en la política de datos (Ley 1581). Caso de prueba: P1.10 en PROCESO-AUXILIAR-CONTABLE.md.
